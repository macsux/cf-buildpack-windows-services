...
class Build : NukeBuild
{
    ...
    [Parameter("Directory of target service you want to run")] 
    AbsolutePath TargetServiceDirectory;
    ...
    AbsolutePath BuildDirectory => Solution.GetProject(BuildpackProjectName).Directory / "bin" / Configuration / Framework / Runtime;
    ...
    Target CombineWithTarget => _ => _
        .Requires(() => TargetServiceDirectory)
        .DependsOn(Compile)
        .Executes(() =>
        {
            CopyDirectoryRecursively(TargetServiceDirectory,BuildDirectory, DirectoryExistsPolicy.Merge, FileExistsPolicy.OverwriteIfNewer);
...
//                .EnableNoRestore());
            ...
            var publishDirectory = buildpackProject.Directory / "bin" / Configuration;
